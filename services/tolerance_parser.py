...
def parse_tolerance(text: str) -> dict:
    ...
    # ----------------------------------------------------------------
    # C. Simple numeric with engineering prefix (R, Ø)
    # ----------------------------------------------------------------
    prefix_regex = r'([ØRΦØM]\d+(?:\.\d+)?)'
    prefix_match = re.search(prefix_regex, clean_text)
    if prefix_match:
        return {
            "dim":  prefix_match.group(1).strip(),
            "utol": "0", "ltol": "0",
        }
    ...
    # ----------------------------------------------------------------
    bare_regex = r'(\d+(?:\.\d+)?)'
    bare_match = re.search(bare_regex, clean_text)
    if bare_match:
        return {
            "dim":  bare_match.group(1).strip(),
            "utol": "0", "ltol": "0",
        }
